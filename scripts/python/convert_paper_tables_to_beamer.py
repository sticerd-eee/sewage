...
GENERATED_MARKER = "% Generated by scripts/python/convert_paper_tables_to_beamer.py"
SOURCE_LINE_RE = re.compile(
    r"^% Source: output/tables/(?P<source_name>[^/\n]+\.tex)\s*$",
    re.MULTILINE,
)
...
def make_output_header(source_name: str) -> str:
    return f"{GENERATED_MARKER}\n% Source: output/tables/{source_name}\n"


def targets_from_source_dir(source_dir: Path) -> list[ConversionTarget]:
    return [
        ConversionTarget(source_name=source_path.name, target_name=source_path.name)
        for source_path in sorted(source_dir.glob("*.tex"))
    ]


def targets_from_target_manifest(
    source_dir: Path,
    target_dir: Path,
) -> tuple[list[ConversionTarget], list[str]]:
    targets: list[ConversionTarget] = []
    skipped: list[str] = []

    for target_path in sorted(target_dir.glob("*.tex")):
        target_name = target_path.name
        text = target_path.read_text(encoding="utf-8")
        source_match = SOURCE_LINE_RE.search(text)

        if source_match is not None:
            source_name = source_match.group("source_name")
        elif (source_dir / target_name).exists():
            source_name = target_name
        else:
            skipped.append(target_name)
            continue

        targets.append(ConversionTarget(source_name=source_name, target_name=target_name))
...
def convert_targets(
    ...
    for target in targets:
        ...
        if not source_path.exists():
            errors.append((target.target_name, f"Missing source file: {source_path}"))
            continue

        try:
            converted = convert_table(source_path.read_text(encoding="utf-8"))
        except Exception as exc:  # noqa: BLE001
            errors.append((target.target_name, str(exc)))
            continue

        destination = target_dir / target.target_name
        destination.write_text(
            make_output_header(target.source_name) + converted,
            encoding="utf-8",
        )

    print(f"Converted {len(targets) - len(errors)} tables.")
    print(f"Wrote slide tables to {target_dir}.")

    if errors:
        print("\nConversion errors:", file=sys.stderr)
        for target_name, message in errors:
            print(f"  - {target_name}: {message}", file=sys.stderr)
        return 1
...
def convert_directory(source_dir: Path, target_dir: Path) -> int:
    return convert_targets(
        ...
        targets=targets_from_source_dir(source_dir),
    )


def convert_target_manifest(source_dir: Path, target_dir: Path) -> int:
    targets, skipped = targets_from_target_manifest(source_dir=source_dir, target_dir=target_dir)

    if skipped:
        print("Skipped non-generated slide tables:")
        for target_name in skipped:
            print(f"  - {target_name}")

    return convert_targets(source_dir=source_dir, target_dir=target_dir, targets=targets)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Regenerate only the .tex files currently in the target directory. "
            "Existing %% Source headers preserve aliases; files without a matching "
            "source are skipped."
        ),
    )
    return parser
...
def main() -> int:
    ...
    if args.from_target_manifest:
        return convert_target_manifest(source_dir=args.source_dir, target_dir=args.target_dir)
