...
load_data <- function() {
  prior_exposure_load_data(CONFIG, "rental", "site")
}
...
calculate_metrics_by_radius <- function(lookup_dt, events_dt) {
  prior_exposure_calculate_metrics(
    lookup_dt, events_dt, "rental", "site", CONFIG$radius_thresholds
  )
}
...
export_data <- function(data) {
  tryCatch({
    ...
    candidate <- prior_exposure_prepare_public(data, "rental", "site")
    publish_prior_exposure_dataset(
      candidate, output_path,
      prior_exposure_public_schema("rental", "site"),
