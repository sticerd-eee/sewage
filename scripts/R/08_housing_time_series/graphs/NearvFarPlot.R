...
site_status <- site_any |>
  ...
  mutate(
    ...
    status = case_when(
      ...
      total_spills > 0 & total_dry == 0 ~ "all_spills",  # renamed from "wet_only"
      total_dry > 0 ~ "has_dry",
      TRUE ~ "no_spill"
    )
  ) |>
...
msoa_counts <- j |>
  group_by(MSOA_NAME) |>
  summarise(
    sites_all_spills = sum(status == "all_spills"),  # renamed
...
p_all      <- plot_red(msoa_agg, "sites_all_spills", "Sites with All Spills", "Sites (count)")
p_all_log  <- plot_red_log(msoa_agg, "sites_all_spills", "Sites with All Spills", "Sites (count)")

p_dry      <- plot_red(msoa_agg, "sites_has_dry",  "Sites with Dry Spill", "Sites (count)")
p_dry_log  <- plot_red_log(msoa_agg, "sites_has_dry",  "Sites with Dry Spill", "Sites (count)")
...
# --- Show ---
print(p_all); print(p_all_log)
print(p_dry); print(p_dry_log)
print(p_nosp); print(p_nosp_log)

# --- Saves ---
ggsave(file.path(out_dir, "msoa_all_spills.png"),       p_all,     width = 9, height = 7, dpi = 300)
ggsave(file.path(out_dir, "msoa_all_spills_log.png"),   p_all_log, width = 9, height = 7, dpi = 300)
