...
#Yorkshire water data has some given as "EA names"
#so coalesce these two columns
yorkshire<-yorkshire%>%
  mutate(site = ifelse(is.na(site), EAsitename, site))
...
consentsyork<-consents_all%>%
  filter(grepl("yorkshire water", COMPANY_NAME, ignore.case = TRUE))%>%
  arrange(DISCHARGE_SITE_NAME, desc(ISSUED_DATE))%>%
  distinct(DISCHARGE_SITE_NAME, .keep_all = TRUE)
#3837


yorkunique<-yorkshire%>%
  distinct(site)
#1913


match1<-left_join(yorkunique, consentsyork, by = c("site" = "DISCHARGE_SITE_NAME"))
#no duplicates
...
edm23<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/yorkshireedmreturn2023.csv")
#2195
...
edm22<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/yorkshireedmreturn2022.csv")
#2221
...
#this final one left is "FALSE" for site name
...
york_list <- list(df1, df2, df3, df4, df5)

yorkindex <- do.call(bind_rows, york_list)%>%
  distinct(site, .keep_all = TRUE)
#1912 as needed ("FALSE" is a true missing value)



yorkshirematched<-left_join(yorkshire, yorkindex, by = "site")

check<-yorkshirematched%>%
  filter(is.na(DISCHARGE_NGR))
#209 entries from the "FALSE" data



save(yorkshirematched, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/yorkshirematched.RData")
...
load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/yorkshirematched.RData")
#611251 raw spill entries

yorkshirematched<-yorkshirematched%>%
  distinct()
#611223


TBC<-yorkshirematched%>%
...
#in these cases, we take site name
...
yblocks1<-create_blocks(yorkshirematched)

yblocks2<-split_blocksv1(yblocks1)



yorkmatched2<-yorkshirematched%>%
  select(-startdatetime, -enddatetime)%>%
  distinct(id, .keep_all = TRUE)

yorkshireclean<-left_join(yblocks2, yorkmatched2, by = "id")

check<-yorkshireclean%>%
  filter(is.na(DISCHARGE_NGR))
#only the FALSE entries


yorkshireclean<-yorkshireclean%>%
  select(- EAnumber, - id)%>%
...
save(yorkshireclean, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/12 24 data/yorkshireclean.RData")
