...
consentssouthern<-consents_all%>%
  filter(grepl("southern water", COMPANY_NAME, ignore.case = TRUE))%>%
  arrange(DISCHARGE_SITE_NAME, desc(ISSUED_DATE))%>%
  distinct(DISCHARGE_SITE_NAME, .keep_all = TRUE)
#2262 permits


southernunique<-southern%>%
  distinct(site)
#1634 sites



match1<-left_join(southernunique, consentssouthern, by = c("site" = "DISCHARGE_SITE_NAME"))
#1634 so no duplicates


missing1<-match1%>%
  filter(is.na(DISCHARGE_NGR))
#1384 so not many matches made



#2023 EDM returns data:

edm23<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/southernedmreturn2023.csv")
#977 only
...
missing3<-match3%>%
  ...
  subset(select=site)
#633 still so quite substantial missing


#match with EDM data from 2022 next 

edm22<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/southernedmreturn2022.csv")
#978
...
#633 so no duplicates


missing4<-match4%>%
  filter(is.na(Site.Name..EA.Consents.Database.))%>%
  subset(select=site)
#629 so very few matches


match5<-left_join(missing4, edm22, by = c("site" = "Site.Name..EA.Consents.Database."))
#629 so no duplicates


missing5<-match5%>%
  filter(is.na(Site.Name..WaSC.operational...optional.))%>%
  select(site)
#629 so no matches whatsoever 



edm21<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/southernedmreturn2021.csv")
#978
...
#48 so no duplicate matches


missing7<-match7%>%
  filter(is.na(Site.Name..WaSC.operational...optional.))%>%
  select(site)
#48 so no matches there


#this is few enough to manually match and use fuzzy matching reliably


#fuzzy matching:

match8<-stringdist_left_join(missing7, consentssouthern, by = c("site" = "DISCHARGE_SITE_NAME"), method = "lv", max_dist = 4, distance_col="distance")%>%
  group_by(site) %>%           
  slice_min(order_by = distance, n = 1) %>%     
  ungroup()
#50 so 2 joint matches
...
write_xlsx(match8, "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/southern unmatched1.xlsx")


#all matched now


match8<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/southernmanual1.csv")
#50 but few approved matches

match9<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/southernmanual2.csv")
#the 41 left
...
southern_list <- list(df1, df2, df3, df4, df5, df6, df7)

southernindex <- do.call(bind_rows, southern_list)%>%
  select(-approved)%>%
  distinct(site, .keep_all = TRUE)
#1634 as needed
...
southernmatched<-left_join(southern, southernindex, by = "site")
checking<-southernmatched%>%
  filter(is.na(DISCHARGE_NGR))
#no missing


save(southernmatched, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/southernmatched.RData")



##now applying 12 24 spill count method
...
load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/southernmatched.RData")
#98017 raw spill entries



southernmatched<-southernmatched%>%
  distinct()
#98017

#check for unermitted discharge sites
TBC<-southernmatched%>%
  filter(grepl("anomaly", PERMIT_NUMBER, ignore.case = TRUE) | grepl("tbc", PERMIT_NUMBER, ignore.case = TRUE) | is.na(PERMIT_NUMBER))
#0

#so we can use permit number as id values

southernmatched<-southernmatched%>%
  mutate(id=PERMIT_NUMBER)
...
southernblocks1<-create_blocks(southernmatched)
#27048

southernblocks2<-split_blocksv1(southernblocks1)
#63462


southernmatched2<-southernmatched%>%
  select(-startdatetime, -enddatetime)%>%
  distinct(id, .keep_all = TRUE)
#845 a lot less than 1634 distinct site names - so several variants of the names relate to one permitted site

southernclean<-left_join(southernblocks2, southernmatched2, by = "id")


finalcheck<-southernclean%>%
  filter(is.na(DISCHARGE_NGR))

southernclean<-southernclean%>%
  select(- EAnumber, - id)%>%
...
save(southernclean, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/12 24 data/southernclean.RData")
