...
setup_logging <- function() {
  log_path <- here::here("output", "log", "11_cross_section_db.log")
...
CONFIG <- list(
  ...
  radius_thresholds = c(10000, 5000, 2000, 1000, 500, 250)
)
...
load_data_to_db <- function(con) {
  ...
  if (!"dat_mo" %in% existing_tables) {
    ...
    sum_na_empty <- function(x, na.rm = FALSE) {
      if (!na.rm) {
        return(sum(x))
      }
    ...
    dat_mo <- dat_mo %>% 
      ...
      group_by(water_company, site_id, year, month) %>% 
      summarise(
        spill_count_mo = sum_na_empty(spill_count_mo, na.rm = TRUE),
        spill_hrs_mo = sum_na_empty(spill_hrs_mo, na.rm = TRUE),
        .groups = "drop"
      ) %>%
...
create_all_years_db <- function(prepared_data, con) {
  ...
  agg_all_years_distances <- house_spill_data_tbl %>%
    ...
    summarise(
      mean_distance = mean(
        if_else(distance_m <= radius, distance_m, NULL),
        na.rm = TRUE
      ),
      min_distance = min(
        if_else(distance_m <= radius, distance_m, NULL),
  ...
  tryCatch(
    {
      ...
      all_years_path <- file.path(CONFIG$processed_dir, "agg_all_yrs_cross_section")
      prior_12mo_path <- file.path(CONFIG$processed_dir, "agg_12mo_cross_section")
