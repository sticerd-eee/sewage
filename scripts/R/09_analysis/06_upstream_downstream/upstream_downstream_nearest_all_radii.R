...
run_all_models <- function(RAD) {
  ...
  notes_count <- make_notes(
    paste0("Spill exposure is measured as the average number of spill events per week (12/24 count) recorded at the nearest overflow within ", RAD, "m from January 2021 to the transaction date.")
  )
  notes_hrs <- make_notes(
    paste0("Spill exposure is measured as the average total number of spill hours per week recorded at the nearest overflow within ", RAD, "m from January 2021 to the transaction date.")
  )
