...
#          property. Sample restricted to properties with exactly one spill
#          site within radius.
#          Models are estimated with and without river distance controls.
#          Each set includes OLS, Controls, MSOA FE, MSOA FE + Controls,
#          LSOA FE, and LSOA FE + Controls. Loop over RAD = 250, 500, 1000.
#
...
#
# Outputs (per radius):
#   - output/tables/hedonic_count_continuous_prior_one_site_{RAD}m.tex
#   - output/tables/hedonic_count_continuous_prior_one_site_distance_{RAD}m.tex
#   - output/tables/hedonic_hrs_continuous_prior_one_site_{RAD}m.tex
#   - output/tables/hedonic_hrs_continuous_prior_one_site_distance_{RAD}m.tex
# ==============================================================================
...
run_all_models <- function(RAD) {
  ...
  cat("\n========================================================\n")
  cat("  Running one-site models for RAD =", RAD, "m\n")
  ...
  cat("    Sales one-site observations:", nrow(dat_one_site_sales), "\n")
  ...
  make_notes <- function(measure_text) {
    paste0(
      "note{}={\\\\footnotesize{\\\\textbf{Notes:} This table presents hedonic estimates of the relationship between sewage spill exposure and property values. The sample is restricted to properties with exactly one storm overflow within ",
      RAD,
      "m in England, 2021--2023. The dependent variable is the log transaction price for sales (columns 1--6) or log weekly asking rent for rentals (columns 7--12). ",
      measure_text,
  ...
  notes_count <- make_notes(
    paste0("Spill exposure is measured as the average number of spill events per week (12/24 count) recorded at the single overflow within ", RAD, "m from January 2021 to the transaction date.")
  )
  notes_hrs <- make_notes(
    paste0("Spill exposure is measured as the average total number of spill hours per week recorded at the single overflow within ", RAD, "m from January 2021 to the transaction date.")
  )
  
  # ==================================================================
  # Table 1: Spill Count, One Site (no distance control)
  # ==================================================================
  ...
  panels_count <- list(
    "House Sales" = list(
      "(1)" = model_sales_count_one_site_1,
      "(2)" = model_sales_count_one_site_1b,
      "(3)" = model_sales_count_one_site_2b,
      "(4)" = model_sales_count_one_site_3b,
      "(5)" = model_sales_count_one_site_2,
      "(6)" = model_sales_count_one_site_3
    ),
    "House Rentals" = list(
      "(7)" = model_rentals_count_one_site_1,
      "(8)" = model_rentals_count_one_site_1b,
      "(9)" = model_rentals_count_one_site_2b,
      "(10)" = model_rentals_count_one_site_3b,
      "(11)" = model_rentals_count_one_site_2,
      "(12)" = model_rentals_count_one_site_3
    )
  )
  ...
  tbl <- modelsummary::modelsummary(
    ...
    notes = " ",
    escape = FALSE,
    title = paste0("Effect of Sewage Spills (Count) on Property Values by Direction: One Site Sample (", rad_label, ")")
  )
  
  tbl <- postprocess_table(
    tbl,
    label = paste0("tbl:hedonic-count-continuous-prior-one-site-", rad_label),
    notes = notes_count
  )
  
  out_path <- file.path(output_dir, paste0("hedonic_count_continuous_prior_one_site_", rad_label, ".tex"))
  writeLines(tbl, out_path)
  record_export(out_path)
  
  # ==================================================================
  # Table 2: Spill Count, One Site + Distance Control
  # ==================================================================
  ...
  panels_count_dist <- list(
    "House Sales" = list(
      "(1)" = model_sales_count_one_site_4,
      "(2)" = model_sales_count_one_site_4b,
      "(3)" = model_sales_count_one_site_5b,
      "(4)" = model_sales_count_one_site_6b,
      "(5)" = model_sales_count_one_site_5,
      "(6)" = model_sales_count_one_site_6
    ),
    "House Rentals" = list(
      "(7)" = model_rentals_count_one_site_4,
      "(8)" = model_rentals_count_one_site_4b,
      "(9)" = model_rentals_count_one_site_5b,
      "(10)" = model_rentals_count_one_site_6b,
      "(11)" = model_rentals_count_one_site_5,
      "(12)" = model_rentals_count_one_site_6
    )
  )
  ...
  tbl <- modelsummary::modelsummary(
    ...
    notes = " ",
    escape = FALSE,
    title = paste0("Effect of Sewage Spills (Count) on Property Values by Direction and Distance: One Site Sample (", rad_label, ")")
  )
  
  tbl <- postprocess_table(
    tbl,
    label = paste0("tbl:hedonic-count-continuous-prior-one-site-distance-", rad_label),
    notes = notes_count
  )
  
  out_path <- file.path(output_dir, paste0("hedonic_count_continuous_prior_one_site_distance_", rad_label, ".tex"))
  writeLines(tbl, out_path)
  record_export(out_path)
  
  # ==================================================================
  # Table 3: Spill Hours, One Site (no distance control)
  # ==================================================================
  ...
  panels_hrs <- list(
    "House Sales" = list(
      "(1)" = model_sales_hrs_one_site_1,
      "(2)" = model_sales_hrs_one_site_1b,
      "(3)" = model_sales_hrs_one_site_2b,
      "(4)" = model_sales_hrs_one_site_3b,
      "(5)" = model_sales_hrs_one_site_2,
      "(6)" = model_sales_hrs_one_site_3
    ),
    "House Rentals" = list(
      "(7)" = model_rentals_hrs_one_site_1,
      "(8)" = model_rentals_hrs_one_site_1b,
      "(9)" = model_rentals_hrs_one_site_2b,
      "(10)" = model_rentals_hrs_one_site_3b,
      "(11)" = model_rentals_hrs_one_site_2,
      "(12)" = model_rentals_hrs_one_site_3
    )
  )
  ...
  tbl <- modelsummary::modelsummary(
    ...
    notes = " ",
    escape = FALSE,
    title = paste0("Effect of Sewage Spills (Hours) on Property Values by Direction: One Site Sample (", rad_label, ")")
  )
  
  tbl <- postprocess_table(
    tbl,
    label = paste0("tbl:hedonic-hrs-continuous-prior-one-site-", rad_label),
    notes = notes_hrs
  )
  
  out_path <- file.path(output_dir, paste0("hedonic_hrs_continuous_prior_one_site_", rad_label, ".tex"))
  writeLines(tbl, out_path)
  record_export(out_path)
  
  # ==================================================================
  # Table 4: Spill Hours, One Site + Distance Control
  # ==================================================================
  ...
  panels_hrs_dist <- list(
    "House Sales" = list(
      "(1)" = model_sales_hrs_one_site_4,
      "(2)" = model_sales_hrs_one_site_4b,
      "(3)" = model_sales_hrs_one_site_5b,
      "(4)" = model_sales_hrs_one_site_6b,
      "(5)" = model_sales_hrs_one_site_5,
      "(6)" = model_sales_hrs_one_site_6
    ),
    "House Rentals" = list(
      "(7)" = model_rentals_hrs_one_site_4,
      "(8)" = model_rentals_hrs_one_site_4b,
      "(9)" = model_rentals_hrs_one_site_5b,
      "(10)" = model_rentals_hrs_one_site_6b,
      "(11)" = model_rentals_hrs_one_site_5,
      "(12)" = model_rentals_hrs_one_site_6
    )
  )
  ...
  tbl <- modelsummary::modelsummary(
    ...
    notes = " ",
    escape = FALSE,
    title = paste0("Effect of Sewage Spills (Hours) on Property Values by Direction and Distance: One Site Sample (", rad_label, ")")
  )
  
  tbl <- postprocess_table(
    tbl,
    label = paste0("tbl:hedonic-hrs-continuous-prior-one-site-distance-", rad_label),
    notes = notes_hrs
  )
  
  out_path <- file.path(output_dir, paste0("hedonic_hrs_continuous_prior_one_site_distance_", rad_label, ".tex"))
  writeLines(tbl, out_path)
  record_export(out_path)
