...
#   - output/tables/hedonic_count_continuous_prior_<RAD>m.tex
#   - output/tables/hedonic_hrs_continuous_prior_<RAD>m.tex
#
# ==============================================================================
...
run_for_radius <- function(RAD) {
  ...
  # Return the four preferred count models (property controls + MSOA/LSOA FE) so
  # the post-loop step can build a cross-radius robustness summary.
  list(
    sale_msoa = model_sales_count_5, sale_lsoa = model_sales_count_3,
    rent_msoa = model_rental_count_5, rent_lsoa = model_rental_count_3
  )
}
...
# ==============================================================================
# 6. Cross-radius robustness summary (preferred specifications only)
# ==============================================================================
cat("\nBuilding cross-radius robustness summary...\n")

custom_notes_summary <- paste0(
  "note{}={\\\\footnotesize{\\\\textbf{Notes:} This table summarises the robustness of the hedonic spill-count estimates to the house-to-site radius. Each column reports estimates for the sample of properties within the stated radius (250m, 500m, or 1000m) of a storm overflow in England, 2021--2024 for sales and 2021--2023 for rentals (no 2024 rental data are available). Properties are excluded where any overflow within the radius has an incomplete spill record over the exposure window, since measured spill exposure would otherwise be understated. Each cell is the coefficient on the weekly spill count from the fully-saturated specification including property controls and the stated fixed effects, estimated separately for house sale prices (log transaction price) and house rentals (log weekly asking rent). Property controls include type, new build status, and tenure for sales; and type, bedrooms, and bathrooms for rentals. Heteroskedasticity-robust standard errors are reported in parentheses. *** p<0.01, ** p<0.05, * p<0.1.}},"
)
