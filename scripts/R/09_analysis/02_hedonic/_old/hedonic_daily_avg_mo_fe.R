# ==============================================================================
# Cross-Sectional Regression Analysis: Daily Average Spill Measures (Month FE)
# ==============================================================================
#
...
#          month fixed effects added to all specifications.
#          Panel A: Sales (log house prices), Panel B: Rentals (log rental
#          prices). Each panel includes Month FE, Controls + Month FE,
#          LSOA + Month FE, and LSOA + Controls + Month FE.
#
...
#
# Outputs:
#   - output/tables/hedonic_spill_count_daily_avg2.tex
#   - output/tables/hedonic_spill_hrs_daily_avg2.tex
#
# ==============================================================================
...
sales <- import(
  ...
  select(
    ...
    lsoa,
    msoa,
    month_id  # Keep month_id for fixed effects
  ) |>
  mutate(
...
dat_sales_clean <- dat_cs_sales |>
  ...
  mutate(
    ...
    month_id = forcats::as_factor(month_id)  # Convert to factor for FE
  )
...
# ==============================================================================
# Estimate Models: Spill Count Daily Average (with Month FE)
# ==============================================================================
cat("Estimating spill count models with month FE...\n")

# Sales Models
model_sales_count_1 <- fixest::feols(
  log_price ~ spill_count_daily_avg | month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)

model_sales_count_1b <- fixest::feols(
  log_price ~ spill_count_daily_avg + property_type + old_new + duration | month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)

model_sales_count_2 <- fixest::feols(
  log_price ~ spill_count_daily_avg | lsoa + month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)

model_sales_count_3 <- fixest::feols(
  log_price ~ spill_count_daily_avg + property_type + old_new + duration | lsoa + month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)
...
model_rental_count_1b <- fixest::feols(
  log_price ~ spill_count_daily_avg + property_type + bedrooms + bathrooms | month_id,
  data = dat_rental_clean,
  vcov = "hetero"
)
...
model_rental_count_3 <- fixest::feols(
  log_price ~ spill_count_daily_avg + property_type + bedrooms + bathrooms | lsoa + month_id,
  data = dat_rental_clean,
  vcov = "hetero"
)

# ==============================================================================
# Estimate Models: Spill Hours Daily Average (with Month FE)
# ==============================================================================
cat("Estimating spill hours models with month FE...\n")

# Sales Models
model_sales_hrs_1 <- fixest::feols(
  log_price ~ spill_hrs_daily_avg | month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)

model_sales_hrs_1b <- fixest::feols(
  log_price ~ spill_hrs_daily_avg + property_type + old_new + duration | month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)

model_sales_hrs_2 <- fixest::feols(
  log_price ~ spill_hrs_daily_avg | lsoa + month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)

model_sales_hrs_3 <- fixest::feols(
  log_price ~ spill_hrs_daily_avg + property_type + old_new + duration | lsoa + month_id,
  data = dat_sales_clean,
  vcov = "hetero"
)
...
model_rental_hrs_1b <- fixest::feols(
  log_price ~ spill_hrs_daily_avg + property_type + bedrooms + bathrooms | month_id,
  data = dat_rental_clean,
  vcov = "hetero"
)
...
model_rental_hrs_3 <- fixest::feols(
  log_price ~ spill_hrs_daily_avg + property_type + bedrooms + bathrooms | lsoa + month_id,
  data = dat_rental_clean,
  vcov = "hetero"
)
...
table_latex_count <- modelsummary::modelsummary(
  ...
  add_rows = add_rows,
  notes = " ",
  title = "Effect of Sewage Spills (Count) on Property Values (Month FE)"
)
...
table_latex_count <- sub(
  "caption=\\{([^}]*)\\},",
  "caption={\\1},\nlabel={tbl:hedonic-spill-count-daily-month},",
  table_latex_count
)
...
output_path_count <- file.path(output_dir, "hedonic_spill_count_daily_avg2.tex")
...
table_latex_hrs <- modelsummary::modelsummary(
  ...
  add_rows = add_rows,
  notes = " ",
  title = "Effect of Sewage Spills (Hours) on Property Values (Month FE)"
)
...
table_latex_hrs <- sub(
  "caption=\\{([^}]*)\\},",
  "caption={\\1},\nlabel={tbl:hedonic-spill-hrs-daily-month},",
  table_latex_hrs
)
...
output_path_hrs <- file.path(output_dir, "hedonic_spill_hrs_daily_avg2.tex")
...
cat("  - hedonic_spill_count_daily_avg2.tex\n")
cat("  - hedonic_spill_hrs_daily_avg2.tex\n")
