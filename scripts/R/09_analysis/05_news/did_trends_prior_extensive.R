...
#
# Outputs:
#   - output/tables/did_trends_prior_extensive.tex
#
# ==============================================================================
...
CONFIG <- list(
  ...
  output_path = here::here("output", "tables", "did_trends_prior_extensive.tex"),
  table_label = "tbl:did-trends-prior-extensive"
)
...
estimate_models <- function(dat, dat_rental) {
  ...
  models <- list(
    ...
    model_sale_2 = fixest::feols(
      log_price ~ near_bin + near_bin:post | msoa + month_id,
      data = dat,
      vcov = ~lsoa
    ),
    ...
    model_sale_4 = fixest::feols(
      log_price ~ near_bin + near_bin:post | lsoa + month_id,
      data = dat,
      vcov = ~lsoa
    ),
...
export_table <- function(models, comparison) {
  ...
  custom_notes <- paste0(
    ...
    "after August 2022, the peak month for Google Trends searches and news ",
    "coverage of sewage spills. Property controls include type (flat, ",
    "semi-detached, terraced, other), new build status, and tenure for sales; ",
    "and type (bungalow, detached, semi-detached, terraced), bedrooms, and ",
    "bathrooms for rentals. Standard errors clustered at the LSOA level are ",
