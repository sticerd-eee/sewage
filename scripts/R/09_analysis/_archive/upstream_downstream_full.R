...
#          2021-2023 period for houses/rentals upstream or downstream from 
#          the spillsite (not just prior to transaction date).
#          Panel A: Sales (log house prices), Panel B: Rentals (log rental
#          prices). Each panel includes upstream/downstream OLS, Controls, FE,
#          and FE + Controls.
#
...
#   - data/processed/UpstreamDownstream/spill_house_signed_with_lateral.csv - Sales upstream and downstream from spillsite
#
# Outputs:
#   - output/tables/hedonic_count_continuous_updown_full.tex
#   - output/tables/hedonic_hrs_continuous_updown_full.tex
#
# ==============================================================================
...
# Upstream Downstream
path_upstream_downstream <- here::here(
  "data",
  "processed",
  "UpstreamDownstream",
...
upstream_downstream <- import(path_upstream_downstream)

upstream_downstream <- upstream_downstream |>
  mutate(direction = factor(direction, levels = c(-1, 1), labels = c("Upstream","Downstream")),
...
# Here need to check on the aggregation step why: 
# 1. Some observations are missing direction but have a spill count - check how many site_ids are in the upstream_downstream compared to spills
# 2. Some observations are missing count but have direction - perhaps the 250m filtering
# unique site_id in spills = 14500, in upstream_downstream = 9745
spill_sales_collapsed <- gen_panel_sales |>
  filter(!is.na(site_id)) |>
  left_join(spills, by = join_by(site_id, year)) |>
  left_join(upstream_downstream, by = join_by(house_id, site_id)) |>
  filter(
    !is.na(spill_count_yr),
    !is.na(spill_hrs_yr),
    !is.na(direction)
  ) |>
  group_by(house_id, direction) |>
#  group_by(house_id, direction, site_id) |>
  summarise(
...
model_sales_count_1b <- fixest::feols(
  log_price ~ spill_count_daily_avg + signed_dist_m + property_type + old_new + duration,
  data = dat_sales_clean,
  split = ~direction,
  vcov = "hetero"
)
...
# Combined models for joint table - now with split models
panels_count <- list(
  "House Sales" = list(
    "1U" = model_sales_count_1[[1]],
    "1D" = model_sales_count_1[[2]],
    "2U" = model_sales_count_1b[[1]],
    "2D" = model_sales_count_1b[[2]],
    "3U" = model_sales_count_2[[1]],
    "3D" = model_sales_count_2[[2]],
    "4U" = model_sales_count_3[[1]],
    "4D" = model_sales_count_3[[2]]
  ),
  "House Rentals" = list(
    "5U" = model_rental_count_1,
    "5D" = model_rental_count_1,
    "6U" = model_rental_count_1b,
    "6D" = model_rental_count_1b,
    "7U" = model_rental_count_2,
    "7D" = model_rental_count_2,
    "8U" = model_rental_count_3,
    "8D" = model_rental_count_3
  )
)

# Add rows for fixed effects and controls - now with doubled columns
add_rows <- tibble::tribble(
  ~term                , ~`1U`, ~`1D`, ~`2U`, ~`2D`, ~`3U`, ~`3D`, ~`4U`, ~`4D`, 
  ~`5U`, ~`5D`, ~`6U`, ~`6D`, ~`7U`, ~`7D`, ~`8U`, ~`8D`,
  "LSOA FE"            , "No" , "No" , "No" , "No" , "Yes", "Yes", "Yes", "Yes",
  "No" , "No" , "No" , "No" , "Yes", "Yes", "Yes", "Yes",
  "Property controls"  , "No" , "No" , "Yes", "Yes", "No" , "No" , "Yes", "Yes",
...
table_latex_count <- modelsummary::modelsummary(
  ...
  add_rows = add_rows,
  notes = " ",
  title = "Effect of Sewage Spills (Count) on Property Values: Full Period by Direction"
)
...
table_latex_count <- sub(
  "caption=\\{([^}]*)\\},",
  "caption={\\1},\nlabel={tbl:hedonic-count-continuous-full-split},",
  table_latex_count
)
...
table_latex_count <- sub(
  ...
  "\\1colsep=2pt,\ncells   = {font = \\\\fontsize{9pt}{10pt}\\\\selectfont},\n",
  table_latex_count
)
...
output_path_count <- file.path(output_dir, "hedonic_count_continuous_updown_full.tex")
...
panels_count <- list(
  "Upstream" = list(
    "(1)" = model_sales_count_1[[1]],
    "(2)" = model_sales_count_1b[[1]],
    "(3)" = model_sales_count_2[[1]],
    "(4)" = model_sales_count_3[[1]]
  ),
  "Downstream" = list(
    "(1)" = model_sales_count_1[[2]],
    "(2)" = model_sales_count_1b[[2]],
    "(3)" = model_sales_count_2[[2]],
    "(4)" = model_sales_count_3[[2]]
  )
)
...
table_latex_count <- modelsummary::modelsummary(
  ...
  add_rows = add_rows,
  notes = " ",
  title = "Effect of Sewage Spills (Count) on Property Values: Full Period by Direction, House Sales"
)
...
output_path_count <- file.path(output_dir, "hedonic_count_continuous_full.tex")
writeLines(table_latex_count, output_path_count)
...
table_latex_hrs <- sub(
  "caption=\\{([^}]*)\\},",
  "caption={\\1},\nlabel={tbl:hedonic-hrs-continuous-full},",
  table_latex_hrs
)
...
output_path_hrs <- file.path(output_dir, "hedonic_hrs_continuous_full.tex")
...
cat("  - hedonic_count_continuous_updown_full.tex\n")
cat("  - hedonic_hrs_continuous_updown_full.tex\n")