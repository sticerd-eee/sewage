...
load(here("raw", "ea_consents", "consents_all.csv"))
#this is consents_all from the EA consents database

load(here("data", "processed", "raw_spill.RData"))
...
consentswessex<-consents_all%>%
  filter(grepl("wessex water", COMPANY_NAME, ignore.case = TRUE))%>%
...
match1<-left_join(wessexunique, consentswessex, by = c("site" = "DISCHARGE_SITE_NAME"))
#1101 no duplicates
...
# one of these says na - so is missing site name information

#edm 2023 data

edm23<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/wessexedmreturn2023.csv")
#698
...
missing3<-match3%>%
  ...
  subset(select=site)
#only 10 missing (one of which is na)

#can manually match these in R:

#BOURNEMOUTH MAXWELL ROAD seems unpermitted currently?
#but we know the grid reference - SZ 13133 93807
#from https://www.gov.uk/government/publications/bh9-1dq-wessex-water-services-limited-environmental-permit-application-advertisement-eprfb3753hna001/bh9-1dq-wessex-water-services-limited-environmental-permit-application-advertisement-eprfb3753hna001

#LITTLETON PANELL CORNBURY MILL remains unmatched - no information online or in consents data

#so 2 missing (one can be grid referenced)
#rest can match now:


match4<-missing3[-(7:8),]%>%
  as.data.frame() %>%
  mutate(PERMIT_NUMBER = c("400622", "070917", "012129", "102259", "071018", "EPRJB3891EW", "EPRJB3891EW", "EPRJB3793WZ"))

match4<-left_join(match4, consentswessex, by = "PERMIT_NUMBER")
#matched fine

match5<-missing3[(7:8),]%>%
  as.data.frame()%>%
  mutate(DISCHARGE_NGR = c("SZ 13133 93807", NA))


#all but 1 matched - so make index..
...
w_list <- list(df1, df2, df3, df4)

wessexindex <- do.call(bind_rows, w_list)%>%
  distinct(site, .keep_all = TRUE)
...
wessexmatched<-left_join(wessex, wessexindex, by = "site")

check<-wessexmatched%>%
  filter(is.na(DISCHARGE_NGR))
#30 entries all from the one unmatched site


#save data:

save(wessexmatched, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/wessexmatched.RData")
...
load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/wessexmatched.RData")
#859866

wessexmatched<-wessexmatched%>%
  distinct()
#667956
...
#both Bournemouth & Littleton discharge sites (have NA for permit reference)


#in these cases use name for id:
...
wblocks1<-create_blocks(wessexmatched)



wblocks2<-split_blocksv1(wblocks1)



wessexmatched2<-wessexmatched%>%
  select(-startdatetime, -enddatetime)%>%
  distinct(id, .keep_all = TRUE)
#1096

wessexclean<-left_join(wblocks2, wessexmatched2, by = "id")

checking<-wessexclean%>%
  filter(is.na(DISCHARGE_NGR))
#22 all from Littleton (unmatched CSO)


wessexclean<-wessexclean%>%
  select(- EAnumber, - id)%>%
...
save(wessexclean, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/12 24 data/wessexclean.RData")
