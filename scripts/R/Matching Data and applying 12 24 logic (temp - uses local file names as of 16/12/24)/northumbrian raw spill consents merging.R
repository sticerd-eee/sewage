#now that missing values from DST issues etc has been solved
#these scripts aim to match every single edm to EA consents
#script 3: northumbrian water.
rm(list=ls())
gc()



library(dplyr)
library(tidyr)
library(lubridate)
library(stringr)
library(fuzzyjoin)


load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/consents_all.RData")
#this is consents_all from the EA consents database

load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/raw_spill.RData")
#this is all spill data merged


northumbrian<-raw_spill%>%
  filter(wasc=="northumbrian")
#286984


#northumbrian water has no provided EA Permit Reference information - all done via name matching


#some string cleaning in consents data
consents_all<-consents_all%>%
  mutate(COMPANY_NAME=trimws(COMPANY_NAME))%>%
  mutate(COMPANY_NAME=gsub("  "," ",COMPANY_NAME))%>%
  mutate(DISCHARGE_SITE_NAME=trimws(DISCHARGE_SITE_NAME))%>%
  mutate(DISCHARGE_SITE_NAME=gsub(" +", " ", DISCHARGE_SITE_NAME))%>%
  #and remove all punctuation from site names
  mutate(DISCHARGE_SITE_NAME = gsub("[[:punct:]]", "", DISCHARGE_SITE_NAME))%>%
  mutate(PERMIT_NUMBER=trimws(PERMIT_NUMBER))%>%
  mutate(ISSUED_DATE=as.POSIXct(ISSUED_DATE, format = "%Y-%m-%d"))

#also remove any extra spaces and punctuation from spill data
northumbrian<-northumbrian%>%
  mutate(site = trimws(site))%>%
  mutate(site = gsub(" +", " ", site))%>%
  mutate(site = gsub("[[:punct:]]", "", site))

#how many unique sites do we have
northumbrianunique<-northumbrian%>%
  distinct(site)
#1595 sites


#so we'll match this first via exact matches with consents
#secondly with EA consents returns
#thirdly via fuzzy matching with manual verification
#and lastly manual matching of remaining


#northumbrian consents - taking the most recent permit information
consentsnorthumbrian<-consents_all%>%
  filter(grepl("northumbrian", COMPANY_NAME, ignore.case = TRUE))%>%
  arrange(DISCHARGE_SITE_NAME, desc(ISSUED_DATE))%>%
  distinct(DISCHARGE_SITE_NAME, .keep_all = TRUE)
#4194

#match exactly with consents data
match1<-left_join(northumbrianunique, consentsnorthumbrian, by = c("site" = "DISCHARGE_SITE_NAME"))

missing1<-match1%>%
  filter(is.na(DISCHARGE_NGR))%>%
  subset(select = site)
#309 missing

df1<-match1%>%
  filter(!is.na(DISCHARGE_NGR))
#1286



#2023 EDM returns data:
edm<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/northumbrianedmreturn2023.csv")
#1565

#simple string cleaning
edm<-edm%>%
  mutate(Site.Name..WaSC.operational...optional.=trimws(Site.Name..WaSC.operational...optional.))%>%
  mutate(Site.Name..WaSC.operational...optional.=gsub(" +", " ", Site.Name..WaSC.operational...optional.))%>%
  mutate(Site.Name..WaSC.operational...optional.=gsub("[[:punct:]]", "", Site.Name..WaSC.operational...optional.))


#matching exactly with WaSC specific naming
matched2<-left_join(missing1, edm, by = c("site" = "Site.Name..WaSC.operational...optional."))
#313 so a few have several matches

matched2<-matched2%>%
  distinct(site, .keep_all = TRUE)
#309 - removed multiple matches


missing2<-matched2%>%
  filter(is.na(Site.Name..EA.Consents.Database.))%>%
  subset(select=site)
#228 still missing

df2<-matched2%>%
  filter(!is.na(Site.Name..EA.Consents.Database.))
#81 matched via this
#a significant amount of these are unpermitted discharges - so no permit number
#permit anomaly



#match exactly with edm return - EA site name now
matched3<-left_join(missing2, edm, by = c("site" = "Site.Name..EA.Consents.Database."))


missing3<-matched3%>%
  filter(is.na(Site.Name..WaSC.operational...optional.))%>%
  subset(select=site)
#224 (only 4 matched)

df3<-matched3%>%
  filter(!is.na(Site.Name..WaSC.operational...optional.))
#4 matched


#quite a few left - so fuzzy matching with manual approval, and then manual 


#matched4<-stringdist_left_join(missing3, consentsnorthumbrian, by = c("site" = "DISCHARGE_SITE_NAME"), method = "lv", max_dist = 4, distance_col="distance")%>%
#  group_by(site) %>%           
#  slice_min(order_by = distance, n = 1) %>%     
#  ungroup()
#237 so some equivalent distance matches - need to go through and approve each

#matched4i<-matched4%>%
#  subset(select = c(site, DISCHARGE_SITE_NAME))



#alternative - see if they match edm from 2022 or 2021 first...

#edm return for 2021
edm21<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/northumbrianedmreturn2021.csv")
#1567


matched4ii<-left_join(missing3, edm21, by = c("site" = "Site.Name..EA.Consents.Database."))%>%
  distinct(site, .keep_all =TRUE)
#234 vs 224 so some joint matches?

nacheck1<-matched4ii%>%
  filter(is.na(Outlet.Discharge.NGR..EA.Consents.Database.))
#only 42 missing...
#so that helped a LOT
#much more effective than fuzzy matching

df4<-matched4ii%>%
  filter(!is.na(Outlet.Discharge.NGR..EA.Consents.Database.))
#182 matched
#two are missing permit references denoted #TBC


#edm return for 2022


edm22<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/northumbrianedmreturn2022.csv")
#1564

missing4<-nacheck1%>%
  distinct(site)

matched5i<-left_join(missing4, edm22, by = c("site" = "Site.Name..EA.Consents.Database."))

nacheck2<-matched5i%>%
  filter(is.na(Outlet.Discharge.NGR..EA.Consents.Database.))
#36 observations left
#so 6 were matched with edm 2022 data

df5<-matched5i%>%
  filter(!is.na(Outlet.Discharge.NGR..EA.Consents.Database.))
#6 matched

#these 36 will have to be matched by supervised (manually approved) fuzzy matching and manual matching

missing5<-nacheck2%>%
  subset(select = site)


#fuzzy matching with EA consents for Northumbrian Water


matched6<-stringdist_left_join(missing5, consentsnorthumbrian, by = c("site" = "DISCHARGE_SITE_NAME"), method = "lv", max_dist = 4, distance_col="distance")%>%
  group_by(site) %>%           
  slice_min(order_by = distance, n = 1) %>%     
  ungroup()
#38 so two joint matches

#very few matched but match quality looks good.

#exporting to excel for manual matching


library(writexl)


write_xlsx(matched6, "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/northumbrian unmatched1.xlsx")



load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/raw_spill.RData")


#dale street cso says it's not on consents database

dale<-consents_all%>%
  filter(grepl("6NW801614", PERMIT_NUMBER))
#none - so need all the information for this observation


howden<-consentsnorthumbrian%>%
  filter(grepl("howden", DISCHARGE_SITE_NAME, ignore.case = TRUE))
#either howden wood sso or cso at manhole 49 howden road
#will check online for more information to determine which

#https://www.nwl.co.uk/globalassets/customer-pdfs/dwmp/dwmp-2023/area-technical-reports/l2_report_tyneside.pdf
#shows the HOWDEN STW CSO INLET (exact name in spill data) drains into "willington quay"
#it is different to CSO at manhole 49 Howden Road - listed separately on their list
#and they have different improvement status
#https://nwlcommunityportal.co.uk/Projects/howdon-sewage-treatment-works-upgrade/planned-work
#gives information on planned works

#https://www.floodmapper.co.uk/stations/howdon-stw-cso-inlet-north-bank
#shows HOWDEN STW CSO inlet as measuring in the TYNE ESTRUARY - same as CSO manhole at 49 Howden Road
#so maybe they are actually the same? - check locations

#look for inlet

inlet<-consentsnorthumbrian%>%
  filter(grepl("inlet", DISCHARGE_SITE_NAME, ignore.case = TRUE))
#don't match howden stw cso inlet

howden2<-consents_all%>%
  filter(grepl("235/1804", PERMIT_NUMBER, ignore.case = TRUE))
#matches exactly

#Manhole off Durham Road Next to Sorrento Pizza shop  NWL
#also a problem
#can see on https://www.nwl.co.uk/globalassets/customer-pdfs/dwmp/dwmp-2023/area-technical-reports/l2_report_tyneside.pdf
#but the EA reference is not clear
#seems different to DURHAM ROAD CSO - also listed separately in that document
#found in 2021 returns
#NZ2678255278 location (NGR)

Sorrento<-consents_all%>%
  filter(grepl("NZ2678255278", DISCHARGE_NGR, ignore.case = TRUE))
#none in consents that match - so will use information from EDM return 2021 (unpermitted at least as of 2021)


#western road / rosehill is known as wallsend burn - discovered via EDM manual match then permit reference

#the unpermitted CSOs for Northumbrian have no reference number - not even a unique unpermitted number like welsh water has
#therefore, it may be best to apply the 12 24 spill count functions with the location as the unique ID?



#all matched now - via fuzzy & manual matching
#load this data in


matched6<-read.csv("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/manual matching/northumbrianmanual1.csv")%>%
  filter(Approved == "yes")
  
#36 matched

#two of these have no permit reference - so take these out with location & waterbody information
df6<-matched6[c(13, 24),]%>%
  mutate(EAnumber = NA)

matched6i<-matched6[-c(13, 24),]%>%
  select(site, EAnumber)

consentsnorthumbrian2<-consents_all%>%
  filter(grepl("northumbrian", COMPANY_NAME, ignore.case = TRUE))%>%
  arrange(PERMIT_NUMBER, desc(ISSUED_DATE))%>%
  distinct(PERMIT_NUMBER, .keep_all = TRUE)
#6161



df7<-left_join(matched6i, consentsnorthumbrian2, by = c("EAnumber" = "PERMIT_NUMBER"))
#all matched - 34 


#make the index from these and use to match the raw spill data

df2<-df2%>%
  rename(COMPANY_NAME = Water.Company.Name)%>%
  rename(PERMIT_NUMBER = EA.Permit.Reference..EA.Consents.Database.)%>%
  rename(DISCHARGE_NGR = Outlet.Discharge.NGR..EA.Consents.Database.)%>%
  rename(RECEIVING_WATER = Receiving.Water...Environment..common.name...EA.Consents.Database.)%>%
  select(site, COMPANY_NAME, PERMIT_NUMBER, DISCHARGE_NGR, RECEIVING_WATER)


northumbrianindex<-bind_rows(df1, df2)

df3<-df3%>%
  rename(COMPANY_NAME = Water.Company.Name)%>%
  rename(PERMIT_NUMBER = EA.Permit.Reference..EA.Consents.Database.)%>%
  rename(DISCHARGE_NGR = Outlet.Discharge.NGR..EA.Consents.Database.)%>%
  rename(RECEIVING_WATER = Receiving.Water...Environment..common.name...EA.Consents.Database.)%>%
  select(site, COMPANY_NAME, PERMIT_NUMBER, DISCHARGE_NGR, RECEIVING_WATER)



northumbrianindex<-bind_rows(northumbrianindex, df3)


df4<-df4%>%
  rename(COMPANY_NAME = Water.Company.Name)%>%
  rename(PERMIT_NUMBER = EA.Permit.Reference..EA.Consents.Database.)%>%
  rename(DISCHARGE_NGR = Outlet.Discharge.NGR..EA.Consents.Database.)%>%
  rename(RECEIVING_WATER = Receiving.Water...Environment..common.name...EA.Consents.Database.)%>%
  select(site, COMPANY_NAME, PERMIT_NUMBER, DISCHARGE_NGR, RECEIVING_WATER)


northumbrianindex<-bind_rows(northumbrianindex, df4)

df5<-df5%>%
  rename(COMPANY_NAME = Water.Company.Name)%>%
  rename(PERMIT_NUMBER = EA.Permit.Reference..EA.Consents.Database.)%>%
  rename(DISCHARGE_NGR = Outlet.Discharge.NGR..EA.Consents.Database.)%>%
  rename(RECEIVING_WATER = Receiving.Water...Environment..common.name...EA.Consents.Database.)%>%
  select(site, COMPANY_NAME, PERMIT_NUMBER, DISCHARGE_NGR, RECEIVING_WATER)


northumbrianindex<-bind_rows(northumbrianindex, df5)

df6<-df6%>%
  rename(DISCHARGE_NGR = location)%>%
  rename(RECEIVING_WATER = water)%>%
  rename(PERMIT_NUMBER=EAnumber)%>%
  select(site, PERMIT_NUMBER, DISCHARGE_NGR, RECEIVING_WATER)

northumbrianindex<-bind_rows(northumbrianindex, df6)

df7<-df7%>%
  rename(PERMIT_NUMBER=EAnumber)%>%
  select(-DISCHARGE_SITE_NAME)
  

northumbrianindex<-bind_rows(northumbrianindex, df7)


northumbrianmatched<-left_join(northumbrian, northumbrianindex, by = "site")

#check for NAs
NAcheck<-northumbrianmatched%>%
  filter(is.na(DISCHARGE_NGR))
#none missing

save(northumbrianmatched, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/northumbrianmatched.RData")




#now to tranform this into 12 24 defined spills

load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/matched/northumbrianmatched.RData")
#286984

northumbrianmatched1<-northumbrianmatched%>%
  distinct()
#285663

#with northumbrian - an issue is using the permit reference as an id column
#create a unique id column - that takes permit reference if it exists or site name if it doesn't

#unpermitted
TBC<-northumbrianmatched1%>%
  filter(grepl("anomaly", PERMIT_NUMBER, ignore.case = TRUE) | grepl("tbc", PERMIT_NUMBER, ignore.case = TRUE) | is.na(PERMIT_NUMBER))
#2169


northumbrianmatched1<-northumbrianmatched1%>%
  mutate(PERMIT_NUMBER = ifelse(grepl("anomaly|tbc", PERMIT_NUMBER, ignore.case = TRUE), NA, PERMIT_NUMBER))



TBC2<-northumbrianmatched1%>%
  filter(is.na(PERMIT_NUMBER))
#2169 so it worked in converting all the missing permit numbers to NA


rm(TBC, TBC2)

na_index<-which(is.na(northumbrianmatched1$PERMIT_NUMBER))


northumbrianmatched1<-northumbrianmatched1%>%
  mutate(id = ifelse(is.na(PERMIT_NUMBER), site, PERMIT_NUMBER))

check<-northumbrianmatched1[na_index,]
rm(na_index, check)


#apply 12 24 spill counting logic:


create_blocks<-function(spills){
  #sort spills by their starttime
  spills <- spills %>% arrange(id, startdatetime)
  
  #create an empty dataframe to store 12 24 counted spills
  blocks<-data.frame(
    id=character(),
    block_start = as.POSIXct(character()),
    block_end = as.POSIXct(character()),
    stringsAsFactors = FALSE
  )
  
  #initialise a counter to track progress
  counter<-0
  
  unique_ids <- unique(spills$id)
  for (current_id in unique_ids) {
    spills_id <- spills %>% filter(id == current_id)
    
    #Initialise block variables - needed for 12/24 logic
    current_block_start <- NULL
    current_block_end <- NULL
    
    for(i in 1:nrow(spills_id)) {
      
      #counter for progress tracking
      counter <- counter + 1
      
      # Print count every 20 rows
      if (counter %% 20 == 0) {
        cat(" Processed:", counter, "\n")
      }
      
      #sets some variables representing spill start and stop times to the relevant data
      spill_start <- spills_id$startdatetime[i]
      spill_end <- spills_id$enddatetime[i]
      
      #now applying current logic
      if (is.null(current_block_start)) {
        # this is the case this is the first spill
        current_block_start <- spill_start
        current_block_end <- spill_start + hours(12)  #originally given as min(spill_start+hours(12), spill_end) - but this doesn't make sense right?
        #so this makes the block end 12 hours after the start
        #ADDED LOGIC
        # ~ deals with edge case that the first spill end lies in DST transition time
        if(is.na(current_block_end)) {
          current_block_end<-spill_start + hours(13)
        }
        #if the spill extends beyond this 12 hours...
        #a while loop to keep adding 24 hours until the block encompasses our spill
        while (current_block_end < spill_end) {
          #CHANGED logic
          new_end <- current_block_end + hours(24)
          #again for edge cases that any of these interim block ends lie in DST transition time
          if (is.na(new_end)) {
            current_block_end <- current_block_end + hours(25)
          } else {
            current_block_end <- new_end
          }
          #so in theory, this should attempt to add 24 hours, but if that would lead to NA
          #then it adds 25 hours, 
          #then resumes adding 24 hours after that if needed
          #hopefully this works, dealing with DST elegantly
        }
        blocks <- rbind(blocks, data.frame(id=current_id, block_start = current_block_start, block_end = current_block_end))
      } else {
        #now we go to the cases that are not the first spill
        #for these we have to consider the three possible cases:
        # ~ that they will be fully encompasses by the previous blocks
        # ~ they cause an extention to the previous blocks
        # ~ they are beyond 24 hours after the previous spill block, and so start a new one entirely
        safe_new_end<-current_block_end + hours(24)
        if (is.na(safe_new_end)){
          safe_new_end <- current_block_end + hours(25)
        }
        
        if (spill_end <= current_block_end) {
          # Spill is completely within the previous blocks
          next #skip this spill as it's already  accounted for
        } else if (spill_start <= safe_new_end) {
          # Spill extends the current block ~ since this logic means
          #these observations have an end beyond the previous block end,
          #but also start before 24 hours after the previous block end
          while (current_block_end < spill_end) {
            #NEW ADDITION again to tackle DST missing values (edge cases)
            new_end <- current_block_end + hours(24)
            if (is.na(new_end)) {
              current_block_end <- current_block_end + hours(25)
            } else {
              current_block_end <- new_end
            }
          }
          blocks$block_end[nrow(blocks)] <- current_block_end 
          #^ this may not be what I want, this seems just to amend our current block
          #although - logically it doesn't matter for my next steps tbf
          #*yes this is valid since we split blocks with a separate function
        } else {
          # Start a new block ~ this is the case we are beyond 24 hours since last block
          current_block_start <- spill_start
          current_block_end <- spill_start + hours(12)#NOTE *** fixed 05/08/24
          #NEW LOGIC FOR EDGE CASES CASUED BY DST CHANGE
          if (is.na(current_block_end)) {
            current_block_end <- spill_start + hours(13)  # Adjust to skip DST gap
          }
          while (current_block_end < spill_end) {
            #ALL NEW LOGIC AGAIN FOR DST EDGE CASES
            new_end <- current_block_end + hours(24)
            if (is.na(new_end)) {
              current_block_end <- current_block_end + hours(25)  
            } else {
              current_block_end <- new_end
            }
          }
          blocks <- rbind(blocks, data.frame(id=current_id, block_start = current_block_start, block_end = current_block_end))
        }
      }
    }
  }
  return(blocks)
  
}




split_blocksv1<-function(blocks){
  
  #creates an empty dataframe to store initial for loop
  firstsplit<-data.frame(
    id=character(),
    block_start = as.POSIXct(character()),
    block_end = as.POSIXct(character()),
    duration = numeric(),
    stringsAsFactors = FALSE
  )
  
  #initialise empty variables for the loops
  current_block_start <- NULL
  current_block_end <- NULL
  current_id<-NULL
  
  counter<-0
  
  #loop through our blocks to separate first 12 hour spill blocks
  for(i in 1:nrow(blocks)) {
    
    #counter for progress tracking
    counter <- counter + 1
    
    # Print count every 20 rows
    if (counter %% 20 == 0) {
      cat(" Row number:", counter, "\n")
    }
    
    #assigns variables to individual observations
    current_id<-blocks$id[i]
    current_block_start<-blocks$block_start[i]
    current_block_end<-blocks$block_end[i]
    
    #gives first spill end (always 12 hours)
    #NEW LOGIC NEEDED - if not 12 hours (if NA) then add 13 hours
    firstinterval_end<-blocks$block_start[i]+hours(12)
    
    #deal with the edge case that the first interval (spill) ends in a "missing" time due to DST change
    if (is.na(firstinterval_end)) {
      firstinterval_end <- current_block_start + hours(13)  
    }
    
    
    #set first duration to 12 - as it's 12 hours, even if we add 13 hours (that's only to adjust for DST change)
    firstinterval_duration<-12
    
    #give the remainder duration - for use later in splitting this further
    remainder_duration<-difftime(current_block_end, firstinterval_end, units = "hours")
    #DONT NEED NEW LOGIC HERE AS BLOCK END WILL ALWAYS BE A TRUE VALUE - given create_blocks function
    
    #append 12 hour interval and remainder into dataframe
    firstsplit<-rbind(firstsplit, data.frame(id=current_id, block_start=current_block_start, block_end=firstinterval_end, duration = firstinterval_duration))
    firstsplit<-rbind(firstsplit, data.frame(id=current_id, block_start=firstinterval_end, block_end=current_block_end, duration = remainder_duration))
  }
  
  #separate 12 hour spills from remainder
  
  first12<-firstsplit%>%
    filter(duration==12)
  
  #for remainder - updated filter to remove negatives, not just 0 and 12
  remainder<-firstsplit%>%
    filter(duration!=12 & duration > 0)
  
  
  #create an empty dataframe for second loop
  remainder_split<-data.frame(
    id=character(),
    block_start = as.POSIXct(character()),
    block_end = as.POSIXct(character()),
    duration = numeric(),
    stringsAsFactors = FALSE
  )
  
  
  for(i in 1:nrow(remainder)){
    #loop for splitting up the remainder into the 24 hour spills
    current_id<-remainder$id[i]
    current_block_start<-remainder$block_start[i]
    current_block_end<-remainder$block_end[i]
    
    #this gives the times that separate spills within our remainder
    times <- seq(from = current_block_start, to = current_block_end, by = "24 hours")
    #NEW LOGIC NEEDED - here an interim value could = NA due to DST change
    
    
    
    
    
    
    #this adds in the final endtime - however this logic may cause inconsistency in daylight savings
    if (times[length(times)] != current_block_end) {
      times <- c(times, current_block_end)
    }
    
    #empty dataframe for results of the interval splits within the loop, as we're generating and saving several observations per observation
    intervals <- data.frame(
      id = current_id,
      block_start = times[-length(times)],
      block_end = times[-1],
      duration = NA, #to ensure rbind() can operate correctly (same no. of columns)
      stringsAsFactors = FALSE
    )
    
    remainder_split<-rbind(remainder_split, intervals)
  }
  
  #merge 12 hour blocks with 24 hour blocks
  miniblock<-rbind(first12, remainder_split)%>%
    arrange(id, block_start)
  
  return(miniblock)
}

blocks<-create_blocks(northumbrianmatched1)
#64081 

miniblocks<-split_blocksv1(blocks)
#113 325 spills


northumbrianmatched2<-northumbrianmatched1%>%
  select(-startdatetime, -enddatetime)%>%
  distinct(id, .keep_all = TRUE)


northumbrianclean<-left_join(miniblocks, northumbrianmatched2, by = "id")

check<-northumbrianclean%>%
  filter(is.na(DISCHARGE_NGR))

northumbrianclean<-northumbrianclean%>%
  select(-ISSUED_DATE, -REVOCATION_DATE, - EAnumber, - id)%>%
  mutate(duration = replace_na(duration, 24))%>%
  select(-EAsitename, -misc_code, -unique_id, wasc)%>%
  rename(permit_reference = PERMIT_NUMBER)%>%
  rename(postcode = ADD_OF_DISCHARGE_SITE_PCODE)

load("C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/12 24 data/welshclean.RData")

save(northumbrianclean, file = "C:/Users/danan/Documents/2024 Internship/Circular economy project/Data/Storm overflow/EDM data/output/12 24 data/northumbrianclean.RData")
