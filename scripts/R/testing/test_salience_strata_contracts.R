...
assert_equal(headline$salience_class, c("coastal_not_bathing", "inland", "inland",
  "inland", "bathing", NA, "coastal_not_bathing", "coastal_not_bathing", "bathing"))
assert_equal(classify_salience_coast(coast, coast_rule_m = 10000)$salience_class,
  c("coastal_not_bathing", "coastal_not_bathing", "coastal_not_bathing", "inland",
    "bathing", NA, "coastal_not_bathing", "coastal_not_bathing", "bathing"))
assert_equal(classify_salience_coast(coast, unknown_policy = "exclude")$salience_class,
  c("coastal_not_bathing", "inland", "inland", "inland", "bathing", NA, NA, NA, NA))
...
assert_equal(classify_salience_coast(radius_coast, 10000, "exclude", "radius")$salience_class,
             c("coastal_not_bathing", "coastal_not_bathing", "coastal_not_bathing",
               "inland", "bathing", NA, NA, NA, NA))
assert_equal(flag_salience_london(tibble::tibble(region = c("London", "South East", NA)))$london,
             c(TRUE, FALSE, FALSE))

# The enumerator exposes three disjoint classes and one explicit coastal union.
coast_filters <- salience_strata("coast_bathing")
assert_equal(names(coast_filters), c("bathing", "coastal_not_bathing", "coastal", "inland"))
masks <- lapply(coast_filters, function(f) f(headline))
assert_equal(masks$coastal, masks$bathing | masks$coastal_not_bathing)
assert_equal(as.integer(masks$bathing) + masks$coastal_not_bathing + masks$inland,
             as.integer(!is.na(headline$salience_class)))
...
assert_equal(joined$salience_class, c("bathing", "bathing", NA, NA))
...
assert_equal(radius_joined$salience_class, c("inland", "inland", "bathing", NA))
...
count_data <- tibble::tibble(
  house_id = c("a", "a", "b", "c", "d", "d"),
  site_id = c(1L, 1L, 2L, 3L, 4L, 4L),
  salience_class = c("bathing", "bathing", "coastal_not_bathing", "inland", NA, NA),
  coast_rule_m = 2000, unknown_policy = "not_designated",
  london = c(TRUE, FALSE, FALSE, FALSE, FALSE, FALSE)
)
...
assert_equal(counts$n_before_london_drop, c(2L, 1L, 3L, 1L))
assert_equal(counts$n_after_london_drop, c(1L, 1L, 2L, 1L))
assert_equal(counts$nearest_site_missing_coast_share, rep(0.25, 4))
assert_equal(counts$n_unclassified, rep(2L, 4))
assert_error(log_salience_cells(filter(count_data, salience_class != "inland"),
                               count_nearest, "rentals", "coast_bathing"),
             "rentals.*inland")
intensity_counts <- intensity |>
  mutate(site_id = 1L, salience_class = "bathing", london = FALSE,
         coast_rule_m = 2000, unknown_policy = "not_designated") |>
