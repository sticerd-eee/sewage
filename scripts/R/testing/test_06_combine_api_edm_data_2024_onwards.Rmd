...
initialise_environment <- function() {
  if (!requireNamespace("renv", quietly = TRUE)) {
    ...
    # Consider renv::init() if this is a new project setup
  }
...
#' Clean the combined EDM API data
#' @param combined_data Tibble containing the raw combined data
#' @return Cleaned tibble with standardized data
...
clean_combined_data <- function(combined_data) {
  ...
  # Clean water company names mapping
  snake_case_names <- tolower(gsub(" ", "_", CONFIG$water_company_names))
  name_mapping <- setNames(CONFIG$water_company_names, snake_case_names)
  ...
  cleaned_data <- combined_data %>%
    ...
    ## Rename the parsed columns and remove the original ones
    rename(
      start_time = start_time_og_parsed,
      end_time = end_time_og_parsed
    )

  logger::log_info("Standardised water company names to proper format")
...
```{r}
combined_df$water_company %>% unique()
```
