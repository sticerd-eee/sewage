...
dat_sales <- gen_panel_sales |>
  ...
  left_join(spills, by = join_by(site_id, qtr_id, year))
...
dat_sales_within <- within_panel_sales |>
  left_join(sales, by = join_by(house_id, month_id, qtr_id, year, price)) |>
  left_join(spills, by = join_by(site_id, qtr_id, quarter, year))

## Assemble rent data
dat_rent_within <- within_panel_rent |>
  left_join(rent, by = join_by(rental_id, month_id, qtr_id, year)) |>
  left_join(spills, by = join_by(site_id, qtr_id, quarter, year))
```