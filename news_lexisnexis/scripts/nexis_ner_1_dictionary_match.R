...
#for this script, we use searches 8 and 9
...
#alternative, allowing duplicates
clean_convert_place_data_2 <- function (place_data, name_column = place) {
  ...
  #remove this, when two places have the same name - retain both
  #place_data <- place_data%>%
  #  distinct({{ name_column }}, .keep_all = T)
...
#reading search_8 and search_9 PDFs
#file paths to search_8 and search_9 data:
#(change as needed)
nexis_paths <- c(paste0(dp_path,"/search_8/search_8_1.pdf"), 
                 paste0(dp_path,"/search_9/search_9_1.pdf"))
...
#only allowing matches to news headlines

news_89 <- news_89 %>%
  mutate(head_lower = tolower(heading))

news_89$ipn_matches_heading <- pblapply(
...
### ~ Major Towns and Cities (ONS)

towns_cities_names <- read.csv(paste0(dp_path, "/Major_Towns_and_Cities_Dec_2015_Boundaries_V2_2022_140358522642712462.csv"))
#only 112 
...
river_sf<-read_sf(paste0(dp_path, "/OSRivers_shapefile/WatercourseLink.shp"))
...
#save(news_89, file = paste0(processed, "/searches_89_matched_dict.RData"))