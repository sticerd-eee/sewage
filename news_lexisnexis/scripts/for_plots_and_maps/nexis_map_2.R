#map of dry spills with news coverage - but limited matches to places with 2 words or more
...
ipn_2_places <- rbindlist(
  lapply(seq_len(nrow(news_89)), function(i) {
    df <- news_89$ipn_matches_several_wrd[[i]]
...
ipn_2_places<-ipn_2_places%>%
  filter(str_length(grid1km) != 0)
#just 1371 in all



results <- pblapply(ipn_2_places$grid1km, function(x) {
  parsed <- osg_parse(x)
...
ipn_2_places <- cbind(ipn_2_places, eastnorth_df)

#make sf object
ipn_sf <- st_as_sf(ipn_2_places, coords = c("easting", "northing"), crs = 27700)
...
ui<-fluidPage(
  ...
  div(
    ...
    tags$p(
      ...
      coverage - matched to Index of Place Names with 2 or more words.
