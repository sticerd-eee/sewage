#A Shiny App that plots spills (w/dry spill classification) and media coverage, matched via statistical assisted NER
...
ipn_places <- rbindlist(
  lapply(seq_len(nrow(news_89)), function(i) {
    df <- news_89$ipn_matches_spacy[[i]]
...
#make sf object
ipn_sf <- st_as_sf(ipn_places, coords = c("easting", "northing"), crs = 27700)
...
ui<-fluidPage(
  ...
  div(
    ...
    tags$p(
      ...
      coverage - matched to Index of Place Names assisted by SpaCy.
