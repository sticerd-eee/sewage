...
nearest <- read_csv(file.path(output_dir, "rentals_nearest_bathing_lookup_keys.csv"), show_col_types = FALSE, col_types = cols(rental_id = col_character()))
radius <- read_csv(file.path(output_dir, "rentals_radius250_bathing_lookup_keys.csv"), show_col_types = FALSE, col_types = cols(rental_id = col_character()))
transaction_path <- file.path(data_root, "zoopla/zoopla_rentals.parquet")
transactions <- read_parquet(transaction_path, col_select = c("rental_id", "listing_price", "region", "month_id", "lsoa", "latitude", "longitude", "property_type", "bedrooms", "bathrooms"))
exposure_path <- file.path(data_root, "cross_section/rentals/prior_to_rental/radius=250")
...
baseline <- prepare_hedonic(transactions, exposure, "rentals") |>
  inner_join(select(nearest, rental_id, first_positive_2024), by = "rental_id", relationship = "one-to-one") |>
  filter(!region %in% "London") |>
  left_join(select(transactions, rental_id, month_id), by = "rental_id", relationship = "one-to-one") |>
  mutate(sample = "hedonic_count_continuous_prior_salience_groups")
intensive <- prepare_intensive(transactions, exposure, articles, "rentals", "log_cumulative_articles") |>
  inner_join(select(radius, rental_id, first_positive_2024_only), by = "rental_id", relationship = "one-to-one") |>
  rename(first_positive_2024 = first_positive_2024_only) |>
  filter(!region %in% "London")
extensive <- transactions |>
  filter(month_id >= 1L, month_id <= 36L) |>
  inner_join(nearest, by = "rental_id", relationship = "one-to-one") |>
  ...
  mutate(log_price = log(listing_price), near_bin = as.integer(distance_m <= 500)) |>
  filter(is.finite(log_price), is.finite(log_cumulative_articles), if_all(all_of(c("lsoa", "month_id", "latitude", "longitude", "property_type", "bedrooms", "bathrooms")), ~ !is.na(.x)), !region %in% "London")
...
saved_audits <- bind_rows(lapply(sample_counts$sample, function(sample_name) {
  read_csv(file.path(source_root, "output/logs", paste0(sample_name, "_cell_counts.csv")), show_col_types = FALSE) |>
    filter(market == "rentals", group == "bathing") |>
...
write_csv(sample_counts, file.path(output_dir, "rental_preestimation_bathing_summary.csv"))
final_sample_counts <- bind_rows(lapply(sample_counts$sample, function(sample_name) {
  ...
  model <- readRDS(model_path)$models$rentals$bathing
...
write_csv(final_sample_counts, file.path(output_dir, "rental_final_fitted_bathing_summary.csv"))
write_csv(all_samples |> group_by(sample, month_id) |> summarise(n_bathing_preestimation = n(), n_first_positive_2024_only = sum(first_positive_2024), .groups = "drop"), file.path(output_dir, "rental_preestimation_bathing_by_month.csv"))
write_csv(extensive |> group_by(near_bin) |> summarise(n_bathing_preestimation = n(), n_first_positive_2024_only = sum(first_positive_2024), .groups = "drop"), file.path(output_dir, "rental_extensive_bathing_by_distance_bin.csv"))
...
write_csv(tibble(path = paths, resolved_path = normalizePath(paths), bytes = as.numeric(file.info(paths)$size), modified = as.character(file.info(paths)$mtime)), file.path(output_dir, "rental_preestimation_source_metadata.csv"))
writeLines(c(
  "Rental counts reproduce the current bathing sample before estimator exclusions; total counts match each saved audit exactly.",
  "Final fitted rental membership is reconstructed by removing the exact fixed-effect levels listed in each saved model's fixef_removed field. The retained counts match saved nobs exactly, and the number removed equals the full saved obsRemoved count. No positional row-order assumption is used.",
...
), file.path(output_dir, "rental_preestimation_scope.txt"))
