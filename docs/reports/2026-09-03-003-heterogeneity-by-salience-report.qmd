...
the full exploration: four coast/bathing strata, intensity splits, and three
robustness checks. The earlier exclusive three-way comparison was removed from
this report; its results remain in the dated three-way memo.
...
![The original tidal shoreline and Coastal Site Groups, with Greater London highlighted.](2026-09-03-003-heterogeneity-by-salience-report/original-coastal-classification/england-wales-coastal-classification.png)
...
if (!isTRUE(params$reestimate)) {
  ...
  required <- here::here("output", "regs",
    paste0(c(prefixes, paste0(prefixes, "_groups")), ".rds"))
...
# Historical (pre-paper-group) output prefixes, used by the reproduction audit.
...
original four-way comparison retains its original common missing-coast
exclusion, including for bathing strata.
...
stopifnot(nrow(historical) == 176L)
...
and the five new paper-group bundles and tables. Paper output prefixes are the five historical
